...
const AdherenceLog = () => {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const [elderlyData] = await Promise.all([
        elderlyService.getAllElderly()
      ])
      
      setElderly(elderlyData)
      
      // Load adherence logs for each elderly
      const logs = []
      for (const patient of elderlyData) {
        try {
          ...
          logs.push(...adherenceData.recentLogs)
        } catch (error) {
  ...
  const filteredLogs = adherenceLogs.filter(log => {
    ...
    <div className="p-6">
      ...
      {filteredLogs.length === 0 ? (
        <div className="text-center py-12">
          <div className="text-gray-500 text-lg">No adherence logs found for the selected criteria</div>
        </div>
      ) : (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredLogs.map((log) => (
                  <tr key={log._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {log.takenTime ? new Date(log.takenTime).toLocaleString() : '-'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
